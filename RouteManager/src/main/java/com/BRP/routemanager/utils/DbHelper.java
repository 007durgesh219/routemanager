...
import com.BRP.routemanager.events.DBChangedEvent;
...
public class DbHelper extends SQLiteOpenHelper {
    ...
    public void setTable(Stop[] stops) {
        ...
        for (int i = 0; i < stops.length; i++) {
            Stop stop = stops[i];
            addStop(stop.getStop_pos(), stop.getStop_name(), String.valueOf(stop.getStop_lat()), String.valueOf(stop.getStop_lon()));
        }
        EventBus.getDefault().post(new DBChangedEvent());
    }

    public void addStop(String pos, String stopName, String lat, String lon) {
        try {
